...
import { LoginSchema } from "@/modules/project-auth/auth.schemas";
...
export async function POST(
  ...
  try {
    ...
    const result = await projectAuthService.login(access.schemaName, projectKey, input);
    const response = jsonResponse(request, result);
    applyCors(request, response);
    return response;
  } catch (error) {
